...
# --- PHASE 3: THE WELCOME LETTER ---
    ...
    st.markdown("<h1 style='text-align:center; color:#FFD700;'>✉️ WELCOME TO THE FAMILY</h1>", unsafe_allow_html=True)
    st.markdown("<div style='background-color:#1A1A1A; padding:30px; border-radius:15px; border:1px solid #333;'><h3>Welcome to The Mitrax Command Center!</h3><p>Android: Tap 3 dots (⋮) -> Install App. <br> iPhone: Tap Share -> Add to Home Screen.</p></div>", unsafe_allow_html=True)
    if st.button("ENTER THE MITRAX VAULT", key="btn_vault"):
        st.session_state.step = 'signup'
        st.rerun()

# --- PHASE 4: THE GOLDEN MITRAX VAULT ---
    ...
    with st.form("vault_reg"):
        ...
        if st.form_submit_button("ACTIVATE PYRAMID ACCESS"):
            st.success(f"Welcome to the Vault, {name}!")
    with st.expander("🛠️"):
        if st.text_input("CHEF CODE", type="password") == "Mitrax-Chef":
            st.info("Vault Overridden. Accessing Master Control...")
