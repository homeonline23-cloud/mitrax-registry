...
# --- 3. PHASE 1: SOLDIER DISPLAY (THE INTRO) ---
if st.session_state.step == 'video':
    ...
    # --- THE SOLDIER VIDEO ---
    # Replace the link below with your actual soldier video link!
...
# --- 5. PHASE 3: WELCOME LETTER ---
    ...
    <div style='background-color:#1A1A1A; padding:20px; border-radius:15px; border:1px solid #333;'>
        ...
        <p>Open the link in Chrome (Android) or Safari (iPhone).</p>
        <p><b>Android:</b> 3 dots (⋮) -> "Install App".</p>
...
# --- 6. PHASE 4: SIGN UP ---
    ...
    st.markdown("<h1 class='client-header'>👤 SIGN UP</h1>", unsafe_allow_html=True)
    with st.form("vault_reg"):
        st.text_input("Name")
        st.text_input("Email")
        if st.form_submit_button("ACTIVATE ACCESS"):
            st.success("Accessing 4x Grids...")
